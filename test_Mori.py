...
def test_backup():
    assert opiekun.backup() == "Lorem ipsum dolor sit posuere."

def test_do_something():
    _inicjator.do_something()
    _opiekun.backup()
    assert _inicjator.do_something() != opiekun.backup()

def test_show_history():
    _inicjator = Inicjator("ABCD")
    _opiekun.backup()
    assert _opiekun.show_history() == "Opiekun: Oto lista zapisanych Memento: ABCD"

def test_undo():
    s0 = inicjator.do_something()
    opiekun.backup()
    inicjator.do_something()
    assert opiekun.undo() == s0